...
// ENDPOINTS PARA FACTURAS DE CLIENTES
// 1. OBTENER TODAS LAS FACTURAS
app.get('/api/tech/facturas', verifyTechnician, async (req, res) => {
    try {
        const { page = 1, limit = 10, search = '', cliente = '' } = req.query;
        ...
        `;
        let countQuery = 'SELECT COUNT(*) as total FROM facturaCLI f LEFT JOIN usuarios u ON f.cliente_id = u.id';
        let params = [];
        let whereConditions = [];
        
        if (search) {
            whereConditions.push('(f.numero_factura LIKE ? OR u.nombre LIKE ? OR u.email LIKE ?)');
        ...
        const [facturas] = await connection.execute(query, params);
...
app.get('/api/tech/facturas/:id', verifyTechnician, async (req, res) => {
    try {
        const { id } = req.params;
        const connection = await pool.getConnection();
        
        const [factura] = await connection.execute(`
            SELECT f.*, u.nombre as cliente_nombre, u.email as cliente_email, u.telefono as cliente_telefono
            FROM facturaCLI f 
            LEFT JOIN usuarios u ON f.cliente_id = u.id 
            WHERE f.id = ?
        `, [id]);
...
app.post('/api/tech/facturas', verifyTechnician, async (req, res) => {
    try {
        const { cliente_id, numero_factura, descripcion, monto, estado, fecha_vencimiento } = req.body;
        
        if (!cliente_id || !numero_factura || !monto) {
            return res.status(400).json({
                success: false,
                error: 'Cliente, número de factura y monto son requeridos'
            });
        }
        ...
        const [cliente] = await connection.execute('SELECT id FROM usuarios WHERE id = ?', [cliente_id]);
        if (cliente.length === 0) {
            connection.release();
            return res.status(400).json({
                success: false,
        ...
        const [existing] = await connection.execute('SELECT id FROM facturaCLI WHERE numero_factura = ?', [numero_factura]);
        if (existing.length > 0) {
            connection.release();
            return res.status(400).json({
                success: false,
        ...
        const [result] = await connection.execute(`
            INSERT INTO facturaCLI (cliente_id, numero_factura, descripcion, monto, estado, fecha_vencimiento) 
            VALUES (?, ?, ?, ?, ?, ?)
        `, [cliente_id, numero_factura, descripcion || null, monto, estado || 'pendiente', fecha_vencimiento || null]);
...
app.put('/api/tech/facturas/:id', verifyTechnician, async (req, res) => {
    try {
        const { id } = req.params;
        const { descripcion, monto, estado, fecha_vencimiento } = req.body;
        
        const connection = await pool.getConnection();
        
        // Verificar que la factura existe
        const [existing] = await connection.execute('SELECT id FROM facturaCLI WHERE id = ?', [id]);
        if (existing.length === 0) {
            connection.release();
            return res.status(404).json({
                success: false,
        ...
        await connection.execute(
            `UPDATE facturaCLI SET ${updateFields.join(', ')} WHERE id = ?`,
            updateValues
        );
...
// 5. ELIMINAR FACTURA
app.delete('/api/tech/facturas/:id', verifyTechnician, async (req, res) => {
    try {
        const { id } = req.params;
        const connection = await pool.getConnection();
        
        const [existing] = await connection.execute('SELECT numero_factura FROM facturaCLI WHERE id = ?', [id]);
        if (existing.length === 0) {
            connection.release();
            return res.status(404).json({
                success: false,
        ...
        await connection.execute('DELETE FROM facturaCLI WHERE id = ?', [id]);
        connection.release();
...
app.get('/api/tech/facturas-stats', verifyTechnician, async (req, res) => {
    try {
        const connection = await pool.getConnection();
        
        // Total de facturas
        const [totalFacturas] = await connection.execute('SELECT COUNT(*) as total FROM facturaCLI');
        
        // Facturas por estado
        const [estadoStats] = await connection.execute(`
            SELECT estado, COUNT(*) as cantidad, SUM(monto) as total_monto 
            FROM facturaCLI 
            GROUP BY estado
        `);
        ...
        const [topClientes] = await connection.execute(`
            SELECT u.nombre, u.email, COUNT(f.id) as total_facturas, SUM(f.monto) as total_monto
            FROM usuarios u
            JOIN facturaCLI f ON u.id = f.cliente_id
        ...
        res.json({
            success: true,
            data: {
                totalFacturas: totalFacturas[0].total,
...
async function initializeTechnicians() {
    try {
        ...
        await connection.execute(`
            CREATE TABLE IF NOT EXISTS facturaCLI (
                id INT AUTO_INCREMENT PRIMARY KEY,
                cliente_id INT,
